...
def start(update, context):
    ...
    if context.args:
        ...
        handle_video_request(update, context, video_id)
        return
...
def handle_video_request(update, context, video_id):
    """Handle video request from mini app"""
    user = update.effective_user
    query = update.callback_query
    
    send_typing_action(user.id, context)
    ...
    if not rate_limiter.is_allowed(user.id):
        ...
        if query:
            ...
            context.bot.send_message(user.id, msg, parse_mode=ParseMode.MARKDOWN)
        return
    
    # Check force join
    if not is_user_member(user.id, context):
        keyboard = [
    ...
    try:
        ...
        if query:
            query.answer("✅ Video sent!", show_alert=False)
        ...
        if query:
            query.answer("❌ Please unblock the bot first!", show_alert=True)
        ...
        if query:
            ...
            context.bot.send_message(user.id, error_msg, parse_mode=ParseMode.MARKDOWN)

# ==================== SAVE VIDEOS FROM DATABASE CHANNELS ====================

def save_channel_video(update, context):
    """Save videos from database channels"""
...
def manage_videos(update, context):
...
def list_videos_by_category(update, context, category=None):
    """List videos by category"""
    query = update.callback_query
    query.answer()
...
def show_full_stats(update, context):
...
def button_callback(update, context):
    ...
    # Manage videos
        ...
        manage_videos(update, context)
    
    # List videos by category
    elif data == 'list_adult':
        list_videos_by_category(update, context, 'adult')
    elif data == 'list_movie':
        list_videos_by_category(update, context, 'movie')
    elif data == 'list_series':
        list_videos_by_category(update, context, 'series')
    elif data == 'list_all':
        list_videos_by_category(update, context, None)
    ...
    # Statistics
        ...
        show_full_stats(update, context)
    ...
    # Copy video code
        ...
        if video:
            ...
            query.answer(f"Code: {code}\n\n✅ Copy this!", show_alert=True)
    
    # Delete video
    elif data.startswith('delete_'):
        video_id = data.split('_', 1)[1]
        if db.delete_video(video_id):
            query.answer("✅ Video deleted!", show_alert=True)
            manage_videos(update, context)
        else:
            query.answer("❌ Failed to delete!", show_alert=True)
